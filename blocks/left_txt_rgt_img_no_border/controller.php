...
class LeftTxtRgtImgNoBorderBlockController extends BlockController {
	
	protected $btName = 'Left Txt Right img No Border';
	protected $btDescription = 'This block contains a Title, content and image on the right. The image is clickable.';
	protected $btTable = 'btDCLeftTxtRgtImgNoBorder';
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('LeftTxtRgtImgNoBorderBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('LeftTxtRgtImgNoBorderBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('LeftTxtRgtImgNoBorderBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('LeftTxtRgtImgNoBorderBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('LeftTxtRgtImgNoBorderBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('LeftTxtRgtImgNoBorderBlockController', 'replaceDownloadFileID'),
			$text);
