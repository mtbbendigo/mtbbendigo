...
<?php  } elseif ($vPlayer==1) { ?>
	...
	<div id="youtube<?php  echo $bID?>" class="youtube-container">
		<div id="youtube">
	<?php  if($url['host'] == 'youtu.be') { ?>
		<div class="flex-video"><!-- foundationzurb Flex component -->
			<iframe width="<?php   echo $vWidth; ?>" height="<?php   echo $vHeight; ?>" src="http://www.youtube.com/embed/<?php  echo $url['path']?>/<?php  echo (strpos($url['path'], '@')) ? '@' : '?'; ?>wmode=transparent" frameborder="0"></iframe>
		</div>
	<?php  }else { ?>
		<div class="flex-video"><!-- foundationzurb Flex component -->
			<iframe type="text/html" width="<?php   echo $vWidth; ?>" height="<?php   echo $vHeight; ?>" src="http://www.youtube.com/embed/<?php  echo $query['v']?>/<?php  echo (strpos($query['v'], '@')) ? '@' : '?'; ?>wmode=transparent" frameborder="0"></iframe>
		</div>
	<?php  } ?>
	</div> </div>
<?php  } else { ?>
