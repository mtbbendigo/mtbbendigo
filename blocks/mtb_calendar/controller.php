...
class MtbCalendarBlockController extends BlockController {
	
	protected $btName = 'Mtb Calendar';
	protected $btDescription = '';
	protected $btTable = 'btDCMtbCalendar';
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('MtbCalendarBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('MtbCalendarBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('MtbCalendarBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('MtbCalendarBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('MtbCalendarBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('MtbCalendarBlockController', 'replaceDownloadFileID'),
			$text);
