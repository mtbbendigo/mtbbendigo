...
//	Uncomment Gareth Bakers awesome code to use this, he cant be bothered perfecting this just yet.
// 	if($parentPage->getCollectionParentID() != 0) //0 is concrete5 Parent page id always.
// 	{
// 		echo '<li><a href="' .$canonical.'" target="' . $ni->target .'" class="menu-item">'. $parentPage->getCollectionName() .'</a></li>';
// 	}
	foreach ($navItems as $ni) 
	{
		if($ni->level == 1)
		...
		{	//This is the sub-menu-item
			echo '<li><a href="' . $ni->url . '" target="' . $ni->target . '" class="sub-menu-item">' . $ni->name . '</a></li>';
		}
	}		
	echo '</ul>';
	echo '<div class="secondary-menu-separator"></div>';
	echo '</nav>';
}
