...
/**
 * Lets find out what page is being rendered so it can be determined at what level in the menu this page is at.
 * If the page depth is less than level 1 then show a link back to the parent page.
 */
$parentId=Page::getByID($c->getCollectionParentID());
$nh = Loader::helper('navigation');
$URL = $nh->getCollectionURL($parentId);
...
if($navItemCount > 0)
{
    ...
    echo '<li><a href="' . View::url('/') . '"><strong>Home</strong></a></li>';

    if($parentId->getCollectionID() > 1){
        echo '<li class="divider"></li>';
        echo '<li><a href="' . $URL . '"><strong>' . $parentId->getCollectionName() . '&nbsp;'.json_decode('"\u21b0"').'</strong></a></li>';
    }
    foreach ($navItems as $ni) {
        ...
        if ($ni->isEnabled) {
            ...
            if ($ni->hasSubmenu) //Parent Page
            {
                ...
                if ($pageLevel < $ni->level) {
                    if (!$showChildren && $ni->level == ($pageLevel + 1)) {
                        //echo '<li class="divider"></li>';
                        echo '<li><a href="' . $ni->url . '" target="' . $ni->target . '">' . $ni->name . '</a></li>';
                        if ($ni->isLast) {
                            echo '</ul></li>';
                        }
                    } else if ($showChildren && $pageLevel != $ni->level) {
                        //echo '<li class="divider"></li>';
                        echo '<li><a href="' . $ni->url . '" target="' . $ni->target . '">' . $ni->name . '</a></li>';
                        if ($ni->isLast) {
                            echo '</ul></li>';
