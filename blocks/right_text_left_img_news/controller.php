...
class RightTextLeftImgNewsBlockController extends BlockController {
	
	protected $btName = 'Right Text Left Image News';
	protected $btDescription = 'This block displays a Title, an image positioned to the left and text positioned on the right.';
	protected $btTable = 'btDCRightTextLeftImgNews';
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('RightTextLeftImgNewsBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('RightTextLeftImgNewsBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('RightTextLeftImgNewsBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('RightTextLeftImgNewsBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('RightTextLeftImgNewsBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('RightTextLeftImgNewsBlockController', 'replaceDownloadFileID'),
			$text);
	...
	{
		return $this->field_4_wysiwyg_content;
	}
