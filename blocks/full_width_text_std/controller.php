...
class FullWidthTextStdBlockController extends BlockController {
	
	protected $btName = 'Full Width Text Std';
	protected $btDescription = 'This block contains just text which spans the entire width of the block.';
	protected $btTable = 'btDCFullWidthTextStd';
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('FullWidthTextStdBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('FullWidthTextStdBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('FullWidthTextStdBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('FullWidthTextStdBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('FullWidthTextStdBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('FullWidthTextStdBlockController', 'replaceDownloadFileID'),
			$text);
