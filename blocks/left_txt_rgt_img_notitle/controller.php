...
class LeftTxtRgtImgNotitleBlockController extends BlockController {
	
	protected $btName = 'Left Text Right Image No Title';
	protected $btDescription = 'This block displays text on the left and image on the right Without a title.';
	protected $btTable = 'btDCLeftTxtRgtImgNotitle';
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('LeftTxtRgtImgNotitleBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('LeftTxtRgtImgNotitleBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('LeftTxtRgtImgNotitleBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('LeftTxtRgtImgNotitleBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('LeftTxtRgtImgNotitleBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('LeftTxtRgtImgNotitleBlockController', 'replaceDownloadFileID'),
			$text);
